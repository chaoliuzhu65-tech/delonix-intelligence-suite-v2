...
function resolveMem0EnvConfig(api: OpenClawPluginApi): unknown {
  ...
  const autoCapture = parseOptionalBoolean(
    getMem0EnvValue(api, "MEM0_AUTO_CAPTURE"),
  );
  const autoRecall = parseOptionalBoolean(
    getMem0EnvValue(api, "MEM0_AUTO_RECALL"),
  );
  const enableGraph = parseOptionalBoolean(
    getMem0EnvValue(api, "MEM0_ENABLE_GRAPH"),
  );
  const searchThreshold = parseOptionalNumber(
    getMem0EnvValue(api, "MEM0_SEARCH_THRESHOLD"),
  );
...
function resolveMem0RawConfig(api: OpenClawPluginApi): unknown {
  if (
    hasConfigObject(api.pluginConfig) &&
...
const memoryPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerCli(
      ({ program }) => {
        ...
        mem0
          ...
          .action(
            async (query: string, opts: { limit: string; scope: string }) => {
              try {
    ...
    function parseAgentIdFromSessionKey(
      ...
      const candidate = (
        colonIndex === -1 ? rest : rest.slice(0, colonIndex)
      ).trim();
    ...
    function resolveHookAgentId(
      ...
      return (
        trimmedExplicit || parseAgentIdFromSessionKey(sessionKey) || "main"
      );
    }
    ...
    if (cfg.autoRecall) {
      api.on("before_agent_start", async (event, ctx) => {
        ...
        const agentId = resolveHookAgentId(
          typeof (ctx as any)?.agentId === "string"
            ? (ctx as any).agentId
            : undefined,
          sessionId,
        );
        ...
        try {
          ...
          const longTermOptions = buildSearchOptions(
            undefined,
            undefined,
            undefined,
            agentId,
          ...
          const longTermResults = await provider.search(query, longTermOptions);
          ...
          api.logger.warn(
            `openclaw-mem0-plugin: recall failed: ${String(err)}`,
          );
        }
      });
    }
    ...
    if (cfg.autoCapture) {
      api.on("agent_end", async (event, ctx) => {
        ...
        try {
          ...
          api.logger.warn(
            `openclaw-mem0-plugin: capture failed: ${String(err)}`,
          );
        }
      });
    }
